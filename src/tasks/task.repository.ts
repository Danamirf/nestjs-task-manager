...
export class TaskRepository extends Repository<Task> {
    ...
    async getTasks(filterDto: GetTasksFilterDto, user: User): Promise<Task[]> {
        ...
        if (search) {
            query.andWhere('(task.title LIKE :search OR task.description LIKE :search)', { search: `%${search}%` } ); // % allows for partial search term
        }
    ...
    async createTask(createTaskDto: CreateTaskDto, user: User): Promise<Task> {
        ...
        try {
            await task.save();
            delete task.user; // deletes user from the task object that is returned
            return task;
        } catch (error) {
